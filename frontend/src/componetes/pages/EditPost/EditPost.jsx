...
const EditPost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`https://printrest-clone-api.onrender.com/api/posts/getPostByID/${postId}`,{
          headers: {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axios.patch(`https://printrest-clone-api.onrender.com/api/posts/EditPost/${postId}`, formData ,{
        headers:{
          "Authorization":`Bearer ${accessToken}`
        }
      });
